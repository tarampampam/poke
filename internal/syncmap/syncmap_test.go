...
func TestSyncMap_KeyChunks(t *testing.T) {
	var (
		m        syncmap.SyncMap[int, struct{}]
		giveKeys = []int{0, 1, 2, 3, 4}
	)

	for _, key := range giveKeys {
		m.Store(key, struct{}{})
	}

	var chunks = m.KeyChunks(2) // get chunks

	require.Equal(t, 3, len(chunks))
	require.Equal(t, 3, cap(chunks))
	...
	for _, chunk := range chunks {
		for _, item := range chunk {
			require.Contains(t, giveKeys, item)

			gotKeys = append(gotKeys, item)
		}
	}

	sort.Ints(gotKeys)

	assert.EqualValues(t, giveKeys, gotKeys)
}

func TestSyncMap_setStruct(t *testing.T) {
...
func TestSyncMap_setMap(t *testing.T) {
