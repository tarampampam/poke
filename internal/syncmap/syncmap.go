...
func (s *SyncMap[K, V]) KeyChunks(chunkSize int) [][]K {
	var (
		keys   = s.Keys()
		chunks = make([][]K, 0, len(keys)/2+1)
	)

	for {
		if len(keys) == 0 {
			break
		} else if len(keys) < chunkSize { // necessary check to avoid slicing beyond slice capacity
			chunkSize = len(keys)
		}

		chunks, keys = append(chunks, keys[0:chunkSize]), keys[chunkSize:]
	}
